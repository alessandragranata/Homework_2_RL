...
class Iiwa_pub_sub : public rclcpp::Node
{
    public:
        ...
        node_handle_(std::shared_ptr<Iiwa_pub_sub>(this))
        {
            ...
            std::cout<<"Press 1 for operational space inverse dynamics  controller."<<std::endl;
            std::cin>>choice; 
            ...
            //Run the chosen trajectory type
            if (trajectory_type == "trapezoidal")
            {   
                VAL=1;
                // Creates a linear trajectory with the trapezoidal profile
                p = planner_.compute_trajectory(t,VAL); // Function that implements the linear trajectory
            }
            else if (trajectory_type == "cubic")
            {   
                VAL=0;
                // Create a linear trajectory with the cubic polynomial
                p = planner_.compute_trajectory(t,VAL); // Function that implements the circular trajectory
            }
            else
            {
        ...
        void cmd_publisher(){
            ...
            if (t_ < total_time){
                ...
                if(cmd_interface_ == "position"){
                    ...
                    // desired Position, velocity and acceleration in the cartesian space
